...
namespace day_12
{
    ...
    struct Position2I
    {
        ...
        Position2I operator-(const Position2I &rhs) const { return {x - rhs.x, y - rhs.y}; }
        int manhattanDistanceFrom(const Position2I &rhs)
        {
            Position2I pos{*this - rhs};
    ...
    public:
        ...
        vector<Position2I> neighbouringPositionsOf(Position2I pos) const
        {
            ...
            if (pos.y - 1 >= 0)
                neighbours.push_back(pos + Position2I::up());
            if (pos.y + 1 <= maxHeight)
                neighbours.push_back(pos + Position2I::dn());
            if (pos.x - 1 >= 0)
                neighbours.push_back(pos + Position2I::lt());
            if (pos.x + 1 <= maxWidth)
                neighbours.push_back(pos + Position2I::rt());
    ...
    class AStar
    {
        struct Node
        {
            ...
            Node(
                ...
                : value(value), cheapestCostToNode(cheapestCostToNode), bestGuessCostToFinish(bestGuessCostToFinish), cameFrom(nullptr)
            {
            }
#endif
#endif
    ...
    public:
        AStar(
            ...
            : estimateDistanceToTarget(estimateDistanceToTargetFn), isTarget(isTargetFn), getNeighboursOf(getNeighboursOfFn), distanceToMove(distanceToMoveFn)
        {
        }
    ...
    string answer_a(const vector<string> &input_data)
    {
        ...
        for (const string &str : input_data)
        {
            for (char c : str)
            {
                ...
                if (c == 'S')
                {
                    indexStart = nodes.size() - 1;
                }
                if (c == 'E')
                {
                    indexEnd = nodes.size() - 1;
                }
            }
        }
        ...
        if (path.has_value())
            return to_string(path.value().size() - 1);
    ...
    string answer_b(const vector<string> &input_data)
    {
        ...
        if (path.has_value())
            possiblePaths.push_back(path.value());

        auto it = min_element(begin(possiblePaths), end(possiblePaths), [](auto &pathA, auto &pathB)
    ...
    string answer_b_1(const vector<string> &input_data)
    {
        ...
        for (const string &str : input_data)
        {
            for (char c : str)
            {
                ...
                if (c == 'S' || c == 'a')
                {
                    indexesStart.push_back(nodes.size() - 1);
                }
                if (c == 'E')
                {
                    indexEnd = nodes.size() - 1;
