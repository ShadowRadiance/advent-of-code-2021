...
RSpec.describe ReactorCore do
  ...
  context "with the example data" do
    ...
    context "before rebooting" do
      ...
      it "has 20 initialization instructions" do
  ...
  context "with the part 2 example data" do
    let(:data) {
      <<~DATA
    ...
    context "after executing all the steps" do
      before { reactor.reboot }
      it "has performed all the ons and offs expected" do
        expect(reactor.on_count).to eq(2758514936282235)
      end
    end
